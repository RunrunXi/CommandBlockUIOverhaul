...
@OnlyIn(Dist.CLIENT)
public class ModCommandBlockScreen extends AbstractModCommandBlockScreen {
    ...
    protected void init() {
        super.init();
        int buttonY = this.height / 6 * 5 - 12;
        this.modeButton = this.addRenderableWidget(CycleButton.<CommandBlockEntity.Mode>builder((mode) -> {
            ...
            return mutablecomponent;
                ...
                .displayOnlyValue().withInitialValue(this.mode)
                .create(this.width / 2 - 50 - 100 - 4, buttonY, 100, 20, Component.translatable("advMode.mode"),
                        (p_169721_, p_169722_) -> {
            this.mode = p_169722_;
        }));
        this.conditionalButton = this.addRenderableWidget(CycleButton.booleanBuilder(Component.translatable("advMode.mode.conditional"), Component.translatable("advMode.mode.unconditional"))
                .displayOnlyValue().withInitialValue(this.conditional)
                .create(this.width / 2 - 50, buttonY, 100, 20, Component.translatable("advMode.type"), (p_169727_, p_169728_) -> {
            this.conditional = p_169728_;
        }));
        this.autoexecButton = this.addRenderableWidget(CycleButton.booleanBuilder(Component.translatable("advMode.mode.autoexec.bat"), Component.translatable("advMode.mode.redstoneTriggered"))
                .displayOnlyValue().withInitialValue(this.autoexec)
